    ...
    {W>0&&L>0&&<>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        ...
        <Btn onClick={()=>{
          ...
          <p>${new Date().toLocaleDateString("uk-UA")} · ${W}×${L*cfg.modules}м (${(W*L*cfg.modules).toFixed(1)}м²)${cfg.hasTerrace?` + тераса ${cfg.terraceW}×${cfg.terraceL}м`:""}</p>
          <table><tr><th>Складова</th><th>Сума</th></tr>
          <tr><td>Матеріали</td><td>₴${fmt(matCost)}</td></tr>
          <tr><td>Праця бригади</td><td>₴${fmt(labCost)}</td></tr>
          <tr><td>Накладні</td><td>₴${fmt(overhead)}</td></tr>
          <tr><td><b>Собівартість</b></td><td><b>₴${fmt(totalCost)}</b></td></tr>
          <tr><td colspan="2" style="text-align:center;padding:16px"><span class="price">Ціна: ₴${fmt(salePrice)}</span></td></tr></table>
          <p>Фасад: ${FACADES.find(f=>f.v===cfg.facade)?.l||cfg.facade}</p>
          <p>Комплектація: ${KITS.find(k=>k.v===cfg.kit)?.l||cfg.kit}</p>
